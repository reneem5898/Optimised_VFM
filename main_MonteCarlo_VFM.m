...
%         10) GaussianNoise - list of percentages of Gaussian noise to add to displacement fields (e.g. GaussianNoise = linspace(0.15, 0.5, 11))
%         11) num_MonteCarlo - number of Monte Carlo simulations
%         12) noiseType - "phaseNoise" (noise added to phase offsets) or "dispNoise" (noise added directly to displacements)
%         13) fibreNoise - boolean (TRUE/FALSE) - controls whether or not to add Gaussian noise to the fibre orientations
%
...
if numParam > 1
    ...
    elseif exist(strcat(outDir,'/Dir1.txt'), 'f')
        ...
        orDir = outDir; % Orientation directory
    ...
    if fibreNoise
        ...
        orientation = addNoiseFibres(orDir, orientation, perc);
    end
...
if ~isempty(GaussianNoise)
    ...
    if strcmp(noiseType,'phaseNoise')
        ...
        % Create noise copies
        addNoisePhase(outDir, noiseDir, num_MonteCarlo, GaussianNoise);
    ...
    % Add noise directly to complex displacement data
        ...
        % Create noise copies
        addNoise(outDir, noiseDir, num_MCsim, GaussianNoise);
    end
...
if ~isempty(GaussianNoise)
    ...
    for p = 1:length(GaussianNoise)
        for q = 1:num_MonteCarlo
            
            % Simulation number
            ns = (p-1)*num_MonteCarlo + q;
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% Load displacement field with added Gaussian noise
            if strcmp(noiseType, 'phaseNoise')
                    ...
                    for o = 1:size(zRange,1)
                        ...
                        elseif numParam == 3                         
                            ...
                            while diffPerc > 0.1 && iter < maxIter
                                ...
                                if ~exist('moduli','var')
                                    ...
                                    paramEst = moduli; % Parameters calculated from previous iteration
                                end
                                
                                % Create numeric optimised virtual displacement field
                                ...
                                % Calculate estimated material properties
                            ...
                            % Save results in struct
                            result = struct('G12', G12, 'G13', G13, 'T', T, 'damp', damp, 'uVF1', uVF1, 'uVF2', uVF2, 'uVF3', uVF3, 'iter', iter, 'eta', eta, 'K', K, 'strain', strain);
                            Results(q, countZone) = result;
                        ...
                        elseif numParam == 5
                            ...
                            while diffPerc > 0.1 && iter < maxIter
                                ...
                                paramI = [G12i; G13i; E1i; E3i]; % Only use shear and Young's moduli to calculate convergence
